    ...
    <div class="my-pick">
        <div class="cards-container">
            ...
            >
                ...
                <p class="selection-time">下单时间：{{ formatDate(item.createTime) }}</p>
        ...
        <!-- 订单详细弹窗 -->
        <div v-if="showOrderModal" class="modal-overlay3" @click="closeOrderModal">
            <div class="modal-content3" @click.stop>
                <!-- 循环遍历 selectedMenu 数组 -->
                <div class="modal-section3">
                    <div v-for="(item, index) in selectedMenu" :key="index" class="modal-row3">
                        <img v-if="item.pic" :src="item.pic" :alt="item.title" class="circle-img3">
                        <div class="text-container3">
                            <span v-if="item.title" class="title3">{{ item.title }}</span>
                            <button class="action-btn3" @click="viewRecipeForOrder(item)">查看做法</button>
                        </div>
                    </div>
                </div>
                <button @click="closeOrderModal" class="close-btn-pick3">关闭</button>
            </div>
        </div>

    </div>
    <!-- 做法的弹窗 -->
    <div v-if="showRecipeModal" class="modal-overlay4" @click="closeRecipeModal">
        <div class="modal-content4" @click.stop>
            <h3>{{ recipeTitle }}</h3>
            <img :src="recipePic" class="recipe-img4"/>
            <p v-if="recipeContent">{{ recipeContent }}</p>
            <p v-else>暂无做法</p>
            <button @click="closeRecipeModal" class="close-btn-pick4">关闭</button>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        confirmShare() {
            if (this.friend && this.pick) {
                ...
                const messageData = {
                    ...
                    pick: this.pick,
                    messageContentType: 5,//1.文本 2图片 3甄选 4菜单服务 5订单
                }
        ...
        // 处理"删除"按钮点击事件
        handleDelete(order) {
            this.orderToDelete = order;  // 将需要删除的好友存储起来
            this.showDeleteModal = true;   // 显示弹窗
        },
        // 确认删除订单
        confirmDelete() {
            if (this.orderToDelete) {
                this.deleteOrder(this.orderToDelete);  // 调用删除订单的方法
        ...
        deleteOrder(order) {
            axios
                .put("https://miao.minlan.fun/kitchen/pick/del/" + order.id)
                .then(response => {
                    if (response.data.isOk === 200) {
                        this.fetchData();
                    }
        ...
        closeDeleteModal() {
            ...
            this.orderToDelete = null;    // 清空选择的好友
        },
        handleBack() {
            router.push("/my-page")
        }
