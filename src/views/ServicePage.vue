    ...
    <div class="menu">
        ...
        <div v-if="isDesktop">
            <div class="menu-card" v-for="dish in menu" :key="dish.id">
                <div class="dish-info">
                    <img :src="dish.image" alt="服务照片" class="dish-image" />
        ...
        <div v-if="showAddDishForm" class="modal">
            <div class="modal-content">
                ...
                <form @submit.prevent="editingDish ? updateDish() : addDish()">
                    ...
                    <input v-model="newDish.title" placeholder="服务名称(必须)" required class="form-input" />
                    <input v-model="newDish.description" placeholder="服务描述(必须)" required class="form-input" />
                    <textarea v-model="newDish.method" placeholder="服务程序（可选）" class="form-input" rows="4"></textarea>
                    <!-- 上传图片 -->
...
export default {
    ...
    setup() {
        ...
        const fetchMenuData = async () => {
            try {
                const response = await axios.get("https://miao.minlan.fun/kitchen/menu/getKitchenMenus/2");//大类id是2
                ...
                console.error('获取服务数据时发生错误:', error);
                alert('网络错误，请稍后再试');
            } finally {
        ...
        const deleteDish = async (dishId) => {
            // 弹出确认框，询问是否删除
            const confirmed = window.confirm("确定要删除该服务吗？");
            if (!confirmed) {
                return;  // 用户点击取消时，退出函数
            }
            // 删除操作
        ...
        const fetchCategories = async () => {
            try {
                const response = await axios.get("https://miao.minlan.fun/kitchen/category/loadCategoryByType/2");
                if (response.data.isOk === 200) {
